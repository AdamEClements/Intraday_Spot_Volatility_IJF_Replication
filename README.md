...
- These output files are then used in the R code `IBM_HAC_based_plots.qmd` to create Figure 1. The `.qmd` file requires the `dplyr` and `readr` R packages.
...
- `SACFs.qmd` then uses `SACFs.csv` to create Figure 2. The `.qmd` file requires the `dplyr` and `readr` R packages.
