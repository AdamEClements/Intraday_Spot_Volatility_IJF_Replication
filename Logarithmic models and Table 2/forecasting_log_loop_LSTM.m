...
fore_LSTM = zeros(nfore,npers);     % npers indep. regs.
...
for ii = 1:nfore
    ...
    target(ii,:) = data_tmp((initwin + ii - 1)*npers+1:(initwin + ii)*npers)';
    ...
    if sum(train_grid == ii) > 0
        ...
        res_var = var(yhat - data_tmp_tmp(maxlags*npers+1:end));
...
fore_LSTM1 = reshape(fore_LSTM',nfore*npers,1);
target1 = reshape(target',nfore*npers,1);

tout = table(fore_LSTM1);
writetable(tout,'IBM_lstm_fore_log.csv','WriteVariableNames',true)
