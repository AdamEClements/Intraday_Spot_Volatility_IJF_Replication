...
fileout = sprintf('IBM_multireg_fore_simple_%d_%d.csv',useday,usenight); 
...
for i = 1:nfore
    ...
    [out1]= fore_func(data_tmp_tmp, on_ret_tmp, npers, lags, nlags, maxlags, ...  
                    i_lags, i_nlags, i_maxlags, length(data_tmp_tmp)/npers, on_ret(i + initwin), target(i,:), useday, usenight);
...
tout = table(target1, fore_multireg);
...
function[fore_out] = fore_func(data_tmp, on_ret, npers, lags, nlags, maxlags,...
                        i_lags, i_nlags, i_maxlags, ndays, on_ret_t1, final_day, useday, usenight);
...
% There is not the same fixed number of pars as the model changes
beta_hat_new = cell(npers,1);
