...
OLS_estimates <- OLS_estimates |>
  rename(
    b_0 = betas_1, 
    b_1 = betas_2, b_2 = betas_3, b_3 = betas_4, 
    b_5 = betas_5, b_6 = betas_6, b_7 = betas_7
  ) |>
  mutate(Interval = 1:78, b_123_sum = b_1+b_2+b_3, b_567_sum = b_5+b_6+b_7) |>
  select(Interval, b_0, b_123_sum, b_567_sum)
...
for (i in s) {
    ...
    HAC_SE_b_567_sum[c] <- sqrt(
      covars$covar_robust_5[i+4] + covars$covar_robust_6[i+5] + covars$covar_robust_7[i+6] +
      2*covars$covar_robust_5[i+5] + 2*covars$covar_robust_5[i+6] + 2*covars$covar_robust_6[i+6]  
    )
    c <- c + 1
}

HAC_SEs <- data.frame(HAC_SE_b_0, HAC_SE_b_123_sum, HAC_SE_b_567_sum) |>
